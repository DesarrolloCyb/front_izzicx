...
@Component({
  selector: 'reprocesar-casosnegociosinvalidacion',
  templateUrl: './reprocesar-casosnegociosinvalidacion.component.html',
  styleUrls: ['./reprocesar-casosnegociosinvalidacion.component.scss']
})
export class ReprocesarCasosnegociosinvalidacionComponent implements OnInit {
  showtable:any=[];
  ...
  getAjustesCasoNotDoneSinValidacion(fecha1:any,fecha2:any){
    this.cors.get(`AjustesNotDone/getCasosDeNegocioSinValidacionError`,{
      fecha1:fecha1,
      fecha2:fecha2,
    }).then((response) => {
  ...
  buscar(){
    if(this.formReproceso.controls['fecha'].value == null || this.formReproceso.controls['fecha']?.value[1] == null){
      ...
      this.getAjustesCasoNotDoneSinValidacion(fecha1,fecha2);
    }
  ...
  reprocesar(item:any){
    this.cors.get(`AjustesNotDone/ActualizarStatusCasosDeNegocioSinValidacion`,{
      status:item,
